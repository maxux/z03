...
sqlite3 *db_sqlite_init() {
	...
	printf("[+] lib/sqlite: loading <%s>\n", SQL_DATABASE_FILE);
	
	sqlite3_config(SQLITE_CONFIG_SERIALIZED);
	
	if(sqlite3_open(SQL_DATABASE_FILE, &db) != SQLITE_OK) {
		fprintf(stderr, "[-] lib/sqlite: cannot open sqlite databse: <%s>\n", sqlite3_errmsg(db));
		return NULL;
	}
	
	sqlite3_busy_timeout(db, 30000);
	
	printf("[+] lib/sqlite: database loaded\n");
...
sqlite3_stmt *db_sqlite_select_query(sqlite3 *db, char *sql) {
	...
	if(sqlite3_prepare_v2(db, sql, strlen(sql) + 1, &stmt, NULL) != SQLITE_OK) {
		fprintf(stderr, "[-] lib/sqlite: query <%s> failed: %s\n", sql, sqlite3_errmsg(db));
		return NULL;
	}
...
int db_sqlite_close(sqlite3 *db) {
	...
	if((err = sqlite3_close(db)) != SQLITE_OK) {
		fprintf(stderr, "[-] lib/sqlite: cannot close database: %s\n", sqlite3_errmsg(db));
		
	} else printf("[+] lib/sqlite: database closed.\n");
